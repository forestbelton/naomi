...
const show = (context, playlist) => {
    ...
    db.all('SELECT song.title FROM jukebox_playlists playlist INNER JOIN jukebox_songs song ON playlist.song_id = song.id WHERE playlist.playlist_name = ?', [playlist], (err, rows) => {
        if (err) {
            logger.error(err.toString())
