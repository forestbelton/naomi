...
import { giveBucks, Reason } from '../util/caseBucks'
...
const start = ({ state, message }) => {
    ...
    if (typeof state.blackjackGames[user] !== 'undefined') {
        const playerHand = state.blackjackGames[user].playerHands[Player.PLAYER]
        const yourHand = playerHand.map(formatCard).join(', ')

        message.reply(`you're already playing a game. Your hand: ${yourHand}`)
        return
    }
...
const scores = ({ db, logger, message }) => {
    db.all('SELECT user, SUM(win) AS wins, SUM(lose) AS losses, SUM(win) - SUM(lose) AS score FROM blackjack_scores GROUP BY user ORDER BY score DESC LIMIT 5', (err, rows) => {
        ...
        const maxScoreLength = Math.max(
            rows[0].score.toString().length,
            rows[rows.length - 1].score.toString().length
        )

        const maxUserLength = rows.reduce(
            (name, row) => name.length > row.user.length ? name : row.user,
            ''
        ).length

        const leftPad = (num, len) => Array(len - num.toString().length + 1).join(' ') + num
        const rightPad = (num, len) => num + Array(len - num.toString().length + 1).join(' ')

        const reply = 'the high scores for blackjack are: ```'
            + rows.map(({ user, wins, losses, score }, i) => `${i + 1}. [${leftPad(score, maxScoreLength)}] ${rightPad(user, maxUserLength)} (${wins} wins, ${losses} losses)`).join('\n')
            + '\n```'
...
const playerMove = move => ({ db, kdb, logger, message, state }) => {
    const user = getUser(message)
    const game = state.blackjackGames[user]
    ...
    case GameState.WON:
        ...
        messages.push('You won the game! You received 1 casebuck.')
        ...
        giveBucks({ kdb, logger, user, amount: 1, reason: Reason.BLACKJACK })
