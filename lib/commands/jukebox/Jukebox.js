...
class Jukebox {
    ...
    nextSong(context) {
        ...
        return this.queue.length > 0
            ...
            : new Promise((resolve, reject) => {
                ...
                console.log('playlist null? ' + (playlist === null))

                playlist === null
                    ? context.db.get('SELECT * FROM jukebox_songs WHERE deleted = 0 ORDER BY RANDOM() LIMIT 1', handle)
                    : context.db.get('SELECT * FROM jukebox_songs WHERE deleted = 0 AND id = (SELECT song_id FROM jukebox_playlists WHERE playlist_name = ? ORDER BY RANDOM() LIMIT 1)', [playlist], handle)
            })
    }
	...
	replay(context) {
		...
		if (this.nowPlaying) {
			message.reply(`$this.nowPlaying.title}`)
		}
	}
    ...
    insertIntoPlaylist(context, data) {
