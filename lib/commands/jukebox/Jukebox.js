...
import { checkForSong, deleteSong, songAuthor } from '../../query/jukebox'
...
class Jukebox {
    constructor() {
        ...
        this.blame = this.blame.bind(this)
    }
    ...
    start(context) {
        return new Promise((resolve, _reject) => {
            if (this.playing) {
                ...
                if (state.voiceConnection && voiceChannel.name === state.voiceConnection.name) {
                    this.next(context)
                } else {
                    voiceChannel.join()
                        .then(connection => {
    ...
    blame({ kdb, message, logger }, id) {
        let songId = id

        if (songId === '' && this.nowPlaying) {
            songId = this.nowPlaying.id
        } else if (songId === '') {
            message.reply('I don\'t know which song you want me to look up.')
            return
        }

        songAuthor(kdb)(songId)
            .then(
                ({ author }) => {
                    if (`${author}` === 'null') {
                        message.reply('I don\'t know who added that song.')
                    } else {
                        message.reply(`that song was added by ${author}.`)
                    }
                },
                error => {
                    logger.error(error.toString())
                    message.reply('I had trouble finding that song.')
                }
            )
    }
}
