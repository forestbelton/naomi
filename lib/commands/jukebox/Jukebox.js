...
class Jukebox {
    ...
    next(context) {
        ...
        this.nextSong(context)
            .then(song => {
                ...
                this.timeStartedPlaying = moment().unix()
    ...
    whatsPlaying(context) {
        ...
        if (this.nowPlaying) {
            ...
            const timeLeft = moment.duration(this.timeStartedPlaying + this.nowPlaying.duration - timeNow, 'seconds').humanize()

            message.reply(`${this.nowPlaying.title} is currently playing, ending in ${timeLeft}.`)
        } else {
