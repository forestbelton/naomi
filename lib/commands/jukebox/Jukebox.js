...
import { checkForSong, deleteSong, fetchRandomSong, fetchRandomPlaylistSong, fetchRandomUserSong } from '../../query/jukebox'
...
class Jukebox {
                ...
                stmt.run(
                    ...
                    function(err) {
                        if (err) {
                            ...
                            message.reply(`the download for ${formattedTitle} (#${this.lastID}) completed.`)
                            resolve()
                        }
                    })
            })
    ...
    OGs(context, users) {
        let playingUsers = null

        if (users) {
            const userIds = users.split(' ').map(user => user.match(/<@(.*?)>/)[1])
            playingUsers = userIds
        }
    }
}
