...
class Jukebox {
    ...
    start(context) {
        return new Promise((resolve, _reject) => {
            if (this.playing) {
                ...
                const { client, message, logger, state } = context
    ...
    next(context) {
        const { client, logger, message, state } = context
    ...
    _playSong({ client, state }, song) {
        this.nowPlaying = song
        client.user.setActivity(this.nowPlaying.title)

        this.dispatcher = state.voiceConnection.playOpusStream(
